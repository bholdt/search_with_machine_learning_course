...
def create_vector_query(user_query, size=10):
    query_obj = [user_query]
    embedding = model.encode(query_obj)
    return {
        "size": size,
        "query": {
            "knn": {
...
if __name__ == "__main__":
    ...
    general.add_argument('--vector', help='Use vector search, if set to true')
    ...
    for line in fileinput.input():
        ...
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, index=index_name, vector_query)
