...
def create_query(user_query, click_prior_query, filters, sort="_score", sortDir="desc", size=10, source=None, field_name="name"):
    query_obj = {
        'size': size,
        "sort": [
            {sort: {"order": sortDir}}
                        ...
                        "should": [  #
                            ...
                            },
                            {
                                "multi_match": {
                                    ...
                                    "fields": [f"{field_name}^10", "name.hyphens^10", "shortDescription^5",
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", synonyms=False):
    ...
    field = "name.synonyms" if use_synonyms else "name"
    query_obj = create_query(user_query, click_prior_query=None, filters=None, sort=sort, sortDir=sortDir, source=["name", "shortDescription"], field_name=field)
...
if __name__ == "__main__":
    ...
    general.add_argument('--synonyms',
                         help='Use synonum field with querying')
